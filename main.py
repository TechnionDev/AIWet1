...
def run_astar_for_weights_in_range(heuristic_type: HeuristicFunctionType, problem: GraphProblem, n: int = 30,
    ...
    total_cost = []
    total_nr_expanded = []
    for weight in np.linspace(low_heuristic_weight, high_heuristic_weight, n):
...
def map_problem_experiments():
    ...
    map_problem.set_additional_history_based_data()
    a_star_epsilon = AStarEpsilon(HistoryBasedHeuristic, within_focal_h_sum_priority_function,
                                  focal_epsilon=0.23, max_focal_size=40)
    res = a_star_epsilon.solve_problem(map_problem)
    print(res)
